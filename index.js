...
function swapMinMax(array) {
  ...
  for (let i = 0; i < array[i].leght, i++;) {
    // for (const i  in ) {}// Умова if перевіряє, чи є поточний елемент масиву більшим 
    //за поточне максимальне значення. 
  ...
  [array[minIndex], array[maxIndex]] = [array[maxIndex], array[minIndex]]// Функція повертає масив, в якому мінімальне та максимальне значення обміняні місцями.
  return array;
}
...
function replaceNumbers(arr) {
  ...
  {
    if (arr[i] > 10) {
      result13 = (arr[i] + "Greater than 10");
    }

    else if  {
      result13 = (arr[i] + "Less than or equal to 10");
    }
